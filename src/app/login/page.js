'use client';

import { useState } from 'react';
import Link from 'next/link';
import TextField from '@mui/material/TextField';
import { useForm } from '@/hooks/useForm';
import { useLogin } from '@/hooks/useLogin';
import PasswordField from '@/components/common/PasswordField';
import { useSearchParams } from 'next/navigation';


export default function Login() {
  const searchParams = useSearchParams();
  const { formValues, handleChange } = useForm({
    email: '',
    password: '',
  });

  const { handleLogin, loading, error } = useLogin(formValues);

  return (
    <div
      className="relative min-h-screen bg-cover bg-center bg-no-repeat flex items-center justify-center"
      style={{ backgroundImage: "url('/assets/loginPic.png')" }}
    >
      <div className="absolute inset-0 bg-black/30 backdrop-blur-xs z-0" />
      <div className="relative z-10 bg-[#F9F5F6] w-11/12 sm:w-4/5 md:w-2/3 lg:w-1/2 rounded-2xl p-6 md:p-16">
        <div className="flex justify-center">
          <img src="/assets/LOGO.svg" alt="HRBot Logo" className="w-20 h-20" />
        </div>

        <h2 className="text-2xl font-semibold text-center mb-2">Log in</h2>
        <p className="text-sm text-center mb-6 text-gray-600">
          Don&apos;t have an account?{" "}
          <Link href={`/register?redirect=${searchParams.get("redirect") || ''}`} className="underline font-medium">
            Sign up
          </Link>
        </p>

        <form onSubmit={handleLogin}>
          <div className="mb-4">
            <TextField
              id="email"
              name="email"
              label="Email"
              variant="outlined"
              placeholder="Enter your email"
              value={formValues.email}
              onChange={handleChange}
              fullWidth
              autoComplete="email"
              error={!!error.email}
              helperText={error.email}
            />
          </div>
          <div className="mb-4">
            <PasswordField
              name="password"
              value={formValues.password}
              onChange={handleChange}
              error={!!error.password}
              helperText={error.password}
            />
            <div className="text-right mt-1">
              <Link href="/forgot-password" className="text-sm underline text-gray-700 hover:text-[#468585]">
                Forgot your password
              </Link>
            </div>
          </div>
          {error && (
            <p className="text-red-500 text-sm mt-2 text-center">{error.error}</p>
          )}
          <button
            type="submit"
            disabled={loading}
            className="w-full mt-6 cursor-pointer bg-[#468585] text-white font-semibold py-2 rounded-full hover:bg-[#386969] transition disabled:opacity-50"
          >
            {loading ? "Logging in..." : "Log in"}
          </button>
        </form>
      </div>
    </div>
  );
}
